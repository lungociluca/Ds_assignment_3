...
class Service:
    ...
    def insert_entry(self, form, scenario):
        if scenario == 'user':
            if form.get('new_username') and form.get('password') and form.get('role'):
                user = models.Users(id=None, username=form.get('new_username'),
                                    password=md5_of_string(form.get('password')),
    ...
    def get_device_by_id(self, id):
        return self.device_repository.find_by_id(int(id))
    ...
    def manage_seen_messages_when_viewing(self, sender, receiver):
        sender_id = self.user_repository.find_by_username(sender).ID
        receiver_id = self.user_repository.find_by_username(receiver).ID

        if sender_id != receiver_id:
            if sender_id in self.seen_messages and self.seen_messages[sender_id]:
                self.seen_messages[sender_id].add(receiver_id)
            else:
                self.seen_messages[sender_id] = set()
                self.seen_messages[sender_id].add(receiver_id)

        if receiver_id in self.seen_messages and self.seen_messages[receiver_id]:
            notifications = [f'{self.user_repository.find_by_id(user_id).username} opened your texts!'
                             for user_id in self.seen_messages[receiver_id]]
            return notifications

        return []

    def manage_seen_messages_when_sending(self, sender, receiver):
        sender_id = self.user_repository.find_by_username(sender).ID
        receiver_id = self.user_repository.find_by_username(receiver).ID
        if receiver_id in self.seen_messages and self.seen_messages[receiver_id] and sender_id in self.seen_messages[receiver_id]:
            self.seen_messages[receiver_id] = self.seen_messages[receiver_id].remove(sender_id)
